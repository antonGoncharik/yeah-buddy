...
Недельное «ПН/СР/ПТ динамика» не описывает дневник. Фаза — один проход слотов:

```text
A → статика → B → C → снова A
```

```text
A — ноги, жим сидя, дельты
B — жимы, тяги, шраги
C — арм, динамика
статика — те же арм-упражнения, удержание
```

Следующий слот считается от последней сессии текущей фазы. Дату пользователь выбирает сам. Система не создаёт сессию молча при открытии хаба.

Одна тренировка на дату: `unique(user_id, session_date)`. Динамика и статика в этом дневнике в один день не ставятся.

На динамическую сессию попадают упражнения слота. На статику — активные с `workout_type = both` или `static`. Единица подхода берётся из типа сессии: динамика → повторы, статика → секунды.
...
4. Тренировки идут по ротации слотов A → статика → B → C
...
3. Проценты рабочих подходов — ядро из раздела 4. В UI настраиваются шаг и пресет разминки, не jsonb-карта процентов
...
```text
/workouts                         хаб: фаза, следующий слот, сегодняшняя сессия
/workouts/exercises               список: слот, шаг, рабочий максимум
/workouts/exercises/new           упражнение: слот, шаг, пресет, максимум
/workouts/exercises/[id]          правка, архив
/workouts/macro                   фазы и переход
/workouts/macro/new               максимумы по упражнениям
/workouts/schedule                порядок слотов (ротация)
/workouts/sessions/[id]           подходы: план крупно, «готово» подтверждает план
```

### 13.7. Ротация и сессии

- Одна тренировка на дату: `unique(user_id, session_date)`.
- У сессии есть `slot`: `a | b | c | static`.
- Ротация фазы: A → статика → B → C.
- Сессия не создаётся при GET хаба. Хаб показывает следующий слот и кнопку «Начать сегодня».
- Пользователь может выбрать другой слот — ротация не ломается, следующий считается от последней сессии фазы.

### 13.8. Предложение упражнений на сессию

- Слот A/B/C: активные упражнения этого слота с максимумом фазы.
- Статика: активные `both` или `static` с максимумом фазы.
- Пользователь может убрать или добавить.
- Единица подхода — из типа сессии: динамика → повторы, статика → секунды.
