...
`/today/history`: дни с фактом и планом, бары БЖУ, среднее по загруженным (окно до 7), группировка по месяцам. День открывается как `/today?date=&view=history` — только просмотр, без правки граммов, типа дня и состава.
...
Категории в БД: `base` / `armwrestling` / `isolation`. В форме только база и изоляция, для любого типа (динамика / статика / оба). Значение `armwrestling` — старые строки; UI читает его как базу и при сохранении пишет `base` или `isolation`. В стартовом сиде изоляции нет.
...
Одна тренировка зала и одна запись стола на дату: `unique(user_id, session_date, kind)`, `kind` = `gym` | `table`.
...
В схеме ещё есть `kind = table` (старые записи), но в дневнике и на хабе стола нет: не создаём, не показываем, день питания не трогаем.
...
- разминка открыта, можно свернуть;
...
`/workouts/progress`: кривые по упражнениям. Точки — фактические рабочие из completed-сессий зала (первый рабочий подход). Нет факта — максимумы фаз, иначе глобальные. Фильтр все / база / изоляция; в «база» попадают и старые строки `armwrestling`. Среднее изменение с первых точек.
...
Баннер на «Сегодня»: если есть зал — ссылка на запись (и на отдыхе тоже). Карточка «начать зал» — только у тренировочного дня на сегодня. На отдыхе и пустом дне её нет. Стол в дневнике не показывается.
...
Разбор живёт только в Mini App (`/settings/review`): код считает бриф (средние БЖУ, попадания, топ продуктов, план vs факт, фаза, максимумы). Gemini пишет текст только по брифу. Сырые приёмы и подходы в модель не идут, кроме топа продуктов и коротких заметок сессий. С хаба зала ссылок на разбор и схему подходов нет — только из Настроек.
...
`workout_sessions` — дата, вид, `kind` (`gym` | `table`), `template_id`, nullable макро/фаза. Зал и стол — разные строки на одну дату.
...
```text
base = База
armwrestling = в UI как база (значение только в схеме)
isolation = Изоляция
dynamic = Динамика
static = Статика
both = Оба
