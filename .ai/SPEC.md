...
В форме создания категория не выбирается: тип «оба» → армрестлинг, иначе база. Изоляция в форме не выбирается; в стартовом сиде её нет.
...
История: `/workouts/history` и блок на хабе (последние completed зала). Строка — имя шаблона и факт рабочих (не только дата). Сделанная сессия — только просмотр: факт и заметку уже не правят.
