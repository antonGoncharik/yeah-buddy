...
```text
/workouts                      хаб: очередь, начать зал, стол, фаза
...
/workouts/sessions/[id]        шпаргалка и факт; стол — заметка
...
Стол — отдельная запись без шаблона и без круга. Кнопки «Стол сегодня» на хабе. Фазу и очередь не двигает. День питания помечает тренировочным, если день уже есть.
...
Экран стола: заметка и «Стол был». Отмена: «Стол не был».
...
История: `/workouts/history` и блок на хабе (последние completed). Строка — имя шаблона или «Стол» и факт рабочих (не только дата).
...
Старт сессии зала или стола: если день питания на эту дату уже есть и он отдых — сервер ставит тренировочный тип и цели зала. День питания сам не создаётся.

Баннер на «Сегодня»: если есть зал или стол — ссылка на запись (и на отдыхе тоже). Карточка «начать зал» — только у тренировочного дня на сегодня. На отдыхе и пустом дне её нет.
...
6. Состав сессии зала — шаблон. Круг — `sort_order` активных шаблонов. Стол шаблон не использует и круг не двигает.
7. Сессия не создаётся молча при GET. На дату: один зал и один стол.
