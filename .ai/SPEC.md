...
/settings                           цели БЖУ, тема, продукты, шаблоны еды, история еды, схема подходов, макроцикл, как пользоваться
...
В форме категория: тип «оба» → армрестлинг; динамика и статика — база или изоляция. В стартовом сиде изоляции нет.
...
`/workouts/progress`: кривые по упражнениям. Точки — фактические рабочие из completed-сессий зала (первый рабочий подход). Нет факта — максимумы фаз, иначе глобальные. Фильтр база / арм / изоляция, среднее изменение с первых точек.
