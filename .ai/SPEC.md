...
Зал создаётся только по кнопке «Начать» (хаб, сегодня). Макроцикл и фаза подставляются, если есть; без макро — `macro_cycle_id` и `phase_id` пустые, веса как в разгоне от глобального максимума.
