...
Макро необязателен. Без него круг работает, схема весов — разгон. На хабе блок макроцикла только если он уже заведён. Завести — из Настроек. Перед «Начать» одна строка, откуда сегодняшние веса.
...
Новому пользователю пишется схема **3×5**. Уже сохранённая схема не переписывается. Готовые на экране: 3×5, 5×5, 3×8, пирамида. Личная 88×3 / 82×5 / 76×7 в сид и в готовые не входит.

```text
3×5         80%×5 три раза, сброс 60%×5×3
5×5         80%×5 пять раз
3×8         70%×8 три раза, в наборе ×10
...
На экране схемы пример рывка и сброса считает килограммы от уже поднятого максимума (`max_increase_percent`, 220 + 5% → 230). Разгон и набор — от введённого.
