...
| status | ENUM | запланирована / выполнена / пропущена |
| note | TEXT, NULL | Примечание |
| created_at | TIMESTAMP | Дата создания |
...
**Все формулы настраиваются.** Ниже приведены дефолтные значения на основе данных дневника.

**Веса считаются от максимума фазы:**
```
вес = максимум × процент
```

**Округление:**
- Веса округляются до ближайшего шага (обычно 2.5 кг)
- Пользователь может изменить любой вес вручную

**В статике рабочие веса могут быть выше 100% от максимума.**

---

### 4.2. Динамика: разминка

Разминка одинаковая для всех фаз.

| Подход | Процент | Повторы |
|--------|---------|---------|
| 1 | 50% | 5 |
| 2 | 70% | 3 |
| 3 | 80% | 1 |
...
**Пример (максимум 200):**
```
Разминка 1: 200 × 0.50 = 100 × 5
Разминка 2: 200 × 0.70 = 140 × 3
Разминка 3: 200 × 0.80 = 160 × 1
```

---

### 4.3. Динамика: рабочие подходы

Рабочие подходы зависят от фазы.
...
**Пример (максимум 200):**
```
Рабочий 1: 200 × 0.875 = 175 × 3
Рабочий 2: 200 × 0.80 = 160 × 5
Рабочий 3: 200 × 0.725 = 145 × 7
```
...
**Пример (максимум 200):**
```
Рабочий 1: 200 × 0.875 = 175 × 5
Рабочий 2: 200 × 0.80 = 160 × 5
Рабочий 3: 200 × 0.725 = 145 × 7
```

**Отличие от разгона:** в первом рабочем подходе 5 повторов вместо 3.
...
**Пример (максимум 210):**
```
Рабочий 1: 210 × 0.875 = 183.75 → 182.5 × 3
Рабочий 2: 210 × 0.80 = 168 → 167.5 × 5
Рабочий 3: 210 × 0.725 = 152.25 → 150 × 7
```

**Примечание:** формула рывка такая же, как у разгона, но максимум выше.
...
**Пример (максимум 220):**
```
Рабочий 1: 220 × 0.59 = 129.8 → 130 × 5
Рабочий 2: 220 × 0.59 = 129.8 → 130 × 5
Рабочий 3: 220 × 0.59 = 129.8 → 130 × 5
```

**Примечание:** в сбросе разминка может отсутствовать. Все подходы рабочие.

**Альтернативный вариант сброса (макро 1):**
```
Разминка 1: 50% × 10
Рабочий 1: 50% × 10
Рабочий 2: 50% × 10
...
### 4.4. Статика: разминка

Разминка зависит от фазы.

#### Фазы «разгон», «набор», «рывок»

| Подход | Процент | Секунды |
|--------|---------|---------|
| 1 | 50% | 10 |
| 2 | 80% | 5 (или 3) |
| 3 | 100% | 5 (или 2) |

**Пример (максимум 76, макро 3 разгон):**
```
Разминка 1: 76 × 0.50 = 38 × 10с
Разминка 2: 76 × 0.80 = 60.8 → 60 × 3с
Разминка 3: 76 × 1.00 = 76 × 2с
```

**Примечание:** в макро 1 и 2 секунды в разминке были 5с, в макро 3 стали 3с и 2с. Это настраивается.

#### Фаза «сброс»

Разминка отсутствует, все подходы рабочие.

---

### 4.5. Статика: рабочие подходы

Рабочие подходы зависят от фазы.

**Важно:** в статике рабочие веса обычно выше 100% от максимума (~114%).
...
**Пример (максимум 76, макро 3 разгон):**
```
Рабочий 1: 76 × 1.14 = 86.6 → 87 × 6с
Рабочий 2: 76 × 1.14 = 86.6 → 87 × 6с
Рабочий 3: 76 × 1.14 = 86.6 → 87 × 6с
```
...
**Пример (максимум 76, макро 3 набор):**
```
Рабочий 1: 76 × 1.14 = 86.6 → 87 × 8с
Рабочий 2: 76 × 1.14 = 86.6 → 87 × 6с
Рабочий 3: 76 × 1.14 = 86.6 → 87 × 6с
```

**Отличие от разгона:** в первом рабочем подходе 8 секунд вместо 6.
...
**Пример (максимум 80, макро 3 рывок):**
```
Рабочий 1: 80 × 1.14 = 91.2 → 92 × 6с
Рабочий 2: 80 × 1.14 = 91.2 → 92 × 6с
Рабочий 3: 80 × 1.14 = 91.2 → 92 × 6с
```
...
**Пример (максимум 80, макро 3 сброс):**
```
Рабочий 1: 80 × 0.50 = 40 × 3с
Рабочий 2: 80 × 0.50 = 40 × 3с
Рабочий 3: 80 × 0.50 = 40 × 3с
```

---

### 4.6. Особый случай: сброс с резиной

В фазе «сброс» может быть пометка «динамика с резиной, статика 50%».

Это значит:
- Динамические упражнения выполняются с резиной вместо штанги
- Статические упражнения выполняются с 50% от максимума

В модели данных это отражается через:
- `workout_sessions.note` — пометка о типе тренировки
- `sets.planned_weight` — может быть пустым для резины
...
- Список упражнений на тренировку (на основе фазы и типа тренировки)
- Максимумы для упражнений (копируются из предыдущей фазы)
...
- Заполняет фактические веса и повторения/секунды
...
3. Формулы расчёта весов должны быть настраиваемыми, не зашитыми жёстко
...
11. В статике рабочие веса могут быть выше 100% от максимума (~114%)
12. Разминка и рабочие подходы считаются от максимума фазы
13. Веса округляются до ближайшего шага (обычно 2.5 кг)
...
Формулы не хардкодить в расчёте подходов. Хранить в `workout_settings`:

```text
weight_step numeric default 2.5
max_increase_percent numeric default 5
formulas jsonb
```

`formulas` — карта `dynamic|static` → `ramp|volume|peak|deload` → `{ warmup, work }`, каждый подход:

```json
{ "percent": 87.5, "reps": 3, "seconds": null }
```

Дефолты — из раздела 4 этой спеки. Пользователь может поменять шаг округления и процент роста максимума; правку всей jsonb-карты в UI MVP не делаем, но расчёт читает только настройки из базы.

Создавать `workout_settings` при первом обращении к модулю тренировок.

### 13.5. История максимумов

`global_maxes` и `phase_maxes` только дополняются, строки не обновляются и не удаляются.

- Текущий глобальный максимум упражнения = строка с наибольшим `max_weight`.
- Текущий максимум фазы = последняя строка по `(phase_id, exercise_id)` с наибольшим `set_at`.
- Новый глобальный рекорд пишется, если фактический вес подхода больше текущего рекорда.
- Понижать глобальный рекорд через форму нельзя.
...
```text
/workouts                         хаб: фаза, сегодняшняя тренировка, упражнения
/workouts/exercises               список упражнений и рекорды
/workouts/exercises/new           новое упражнение + начальный максимум
/workouts/exercises/[id]          редактирование, архив, история рекордов
/workouts/macro                   текущий макроцикл, фазы, переход
/workouts/macro/new               первый макроцикл: максимумы по упражнениям
/workouts/schedule                расписание ПН–ВС
/workouts/sessions/[id]           тренировка: упражнения и подходы
```

### 13.7. Расписание и сессии

- Одна тренировка на пользователя в день: `unique(user_id, session_date)`.
- Дефолт расписания: ПН/СР/ПТ — динамика, остальные дни — отдых.
- Запланированная сессия создаётся при открытии хаба, если по расписанию сегодня тренировка, есть текущая фаза и сессии ещё нет.
- Если пользователь потренировался в другой день — создаёт тренировку вручную, расписание не меняется.

### 13.8. Предложение упражнений на сессию

Активные упражнения, у которых `workout_type` совпадает с типом сессии или равен `both`. Пользователь может убрать, добавить и поменять порядок.

Единица подхода берётся из типа **сессии**, не из упражнения: динамика → повторения, статика → секунды.
