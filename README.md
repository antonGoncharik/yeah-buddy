...
- Today opens itself as a rest day from the meal template. Switch to training if you are going to the gym, or copy yesterday onto an empty day. On a filled meal, **Вот обед** sits behind «Ещё»; extra copy actions live there too. Today plus the two previous calendar days stay fully writable in the user's timezone. Empty days up to seven days back can still be filled, marked «догонял»; already logged days in that extra window stay locked. If yesterday is still empty, Today says «Вчера пустой — можно догнать».
...
- Meal templates and the gym queue share separately as snapshots, as a QR or a t.me bot link. A filled meal on Today has **Вот обед** behind «Ещё»: the chat picker opens from the card, four lines in chat, Start. A friend saves the pack and applies it later. Logged days and working weights stay private. The app invite QR lives on the gym hub after the first closed session («Показать программу другу»), not on the done screen and not in Settings.
...
Migrations: `supabase/migrations/0001_init.sql` … `0029_funnel_events.sql` — apply in order in the SQL Editor or with the Supabase CLI.
