...
- Meals: breakfast, lunch, snack / pre–post, dinner. Pick a food, enter grams, get protein / fat / carbs / kcal. A name that is not in your list can be a one-off row: protein / fat / carbs for the portion, not a catalog card. A plate photo lives on the add-food screen when `GEMINI_PLATE_API_KEY` is set: known foods get grams, unknown food becomes that same one-off row. Without that key the diary still logs by hand. Daily photo cap is shown on the plate screen.
- Your own food list: CRUD, search, favorites, recents. After a food shows up most days, the add-food screen offers to favourite it once. A shop catalog can copy a card into that list. No barcodes or live external food APIs.
- Today opens itself as a rest day from the meal template. Switch to training if you are going to the gym, or copy yesterday onto an empty day. On a filled meal, **Вот обед** sits on the card; extra copy actions sit behind «Ещё». Today plus the two previous calendar days stay fully writable in the user's timezone. Empty days up to seven days back can still be filled, marked «догонял»; already logged days in that extra window stay locked. If yesterday is still empty, Today says «Вчера пустой — можно догнать».
...
- Meal templates and the gym queue share separately as snapshots, as a QR or a t.me bot link. A filled meal on Today has **Вот обед**: four lines in chat, Start. A friend saves the pack and applies it later. Logged days and working weights stay private. The app invite QR lives on the gym hub after the first closed session («Показать программу другу»), not on the done screen and not in Settings.
- Ready programs exist as bot objects, no author: `https://t.me/<bot>?start=p_full_body`, `p_five_three_one`, `p_ppl`. `/start` replies with the days and **Поставить**. `@bot` empty query and `@bot 531` / `фуллбади` / `ppl` send the same card. Apply puts the preset, not a personal snapshot. 1ПМ is still asked in the gym.
...
Migrations: `supabase/migrations/0001_init.sql` … `0028_ai_usage.sql` — apply in order in the SQL Editor or with the Supabase CLI.
