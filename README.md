...
- День: отдых или «как в зале». От этого зависят цели БЖУ и какие приёмы пищи видны (на отдыхе полдник, в зале — до/после тренировки).
...
- Новый день собирается из сид-шаблонов еды (в UI их не редактируют) или копируется со вчера.
- История дней питания: БЖУ по дням и среднее. Цели отдыха/тренировки — в настройках.

**Зал**

- Очередь шаблонов (динамика/статика), не расписание по дням недели. Таблица `workout_schedule` в БД есть, продукт на неё не опирается.
...
- Сессия — шпаргалка в зале: веса и подходы из схемы и максимумов. Схему (проценты, разминка, рабочие) правишь в Настройках.
- Прогресс (кривые максимумов), история сессий, справочник упражнений.
...
Миграции: `supabase/migrations/0001_init.sql` … `0007_templates.sql` — по порядку в SQL Editor или через Supabase CLI.
