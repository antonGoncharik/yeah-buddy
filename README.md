...
- Ready programs exist as bot objects, no author: `https://t.me/<bot>?start=p_full_body`, `p_five_three_one`, `p_ppl`. The same three sit on the public landing as pages you can paste anywhere: `/p/full-body`, `/p/531`, `/p/ppl`. Each page has its own preview and a button into that bot link. `/start` replies with the days and **Поставить**. `@bot` empty query and `@bot 531` / `фуллбади` / `ppl` send the same card. Apply puts the preset, not a personal snapshot. 1ПМ is still asked in the gym.
...
Migrations: `supabase/migrations/0001_init.sql` … `0032_waist_and_years.sql` — apply in order in the SQL Editor or with the Supabase CLI.
...
Share links: meal templates and gym queue are separate snapshots (`share_packs`), shown as QR plus a bot `start` link. Apply replaces templates (and food/gym settings), not logged days or working weights. Ready programs use a stable `start=p_<id>` (`full_body`, `five_three_one`, `ppl`) — no pack row — and a public page `/p/full-body`, `/p/531`, `/p/ppl` listed on `/`. The app invite is `https://t.me/<bot>` so a friend lands in the chat and presses Start. The diary routes stay out of the search index; `/` and `/p/*` do not.
