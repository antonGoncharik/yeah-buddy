...
Share links: meal templates and gym queue are separate snapshots (`share_packs`), shown as QR plus a bot `start` link. Apply replaces templates (and food/gym settings), not logged days or working weights. Ready programs use a stable `start=p_<id>` (`full_body`, `five_three_one`, `ppl`) — no pack row. The app invite is `https://t.me/<bot>` so a friend lands in the chat and presses Start. The diary routes stay out of the search index; `/` does not.
